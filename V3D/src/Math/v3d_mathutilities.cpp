...
//       cout << "v = "; displayPoly(v);
   ...
   buildsturm(int ord, SturmSequence<Field>& sseq, RootFindingParameters<Field> const& params)
   {
      ...
      for (k = 2; ; ++k)
      {
         ...
         sp = &sseq[k];
         bool res = modp(*sp0, *sp1, *sp, params);
         if (!res) break;
         /*
...
//               min, max, max - min, nroot, n1, n2);
      ...
      for (its = 0; its < params.maxBisectionIterations; ++its)
      {
         ...
         if (n1 != 0 && n2 != 0) {
            sbisect(np, sseq, min, mid, atmin, atmid, roots, params);
            sbisect(np, sseq, mid, max, atmid, atmax, &roots[n1], params);
            break;
         }
...
namespace V3D
{
                                ...
                                RootFindingParameters<Field> const& params)
   {
      ...
      try
      {
          ...
          */
         min = -4.2; // Use another value than -1 in the (common) case -1 is already a root
         ...
         max = 4.2; // Use another value than +1 in the (common) case +1 is already a root
         ...
         t_bisect.start();
         sbisect(np, sseq, min, max, atmin, atmax, &roots[0], params);
         t_bisect.stop();
      }
      catch (root_finding_internal_error exn)
      {
         return false;
      }
      return true;
